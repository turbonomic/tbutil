# REST API Reference (For Turbonomic 7.17.6)


Date: Tue, 03 Aug 2021 23:13:01 BST

## AddFavoriteScopes

POST scopes by user's uuid

JavaScript Usage ...

	result = client.addFavoriteScopes(user_Uuid, _body)

REST method and path ...

	POST /users/{user_Uuid}/favoritescopes

Swagger tags ...

- [SUPPORTED IN 6.X RELEASES ONLY]
...
Add SAML IDP metadata

JavaScript Usage ...

	result = client.createIdpMetadata()

REST method and path ...

	POST /saml/idpmetadata
...
## CreateKeyStoreFromMultipartFiles

Add SAML keystore

JavaScript Usage ...

	result = client.createKeyStoreFromMultipartFiles()
...
JavaScript Usage ...
	...
	result = client.createSamlConfiguration(_body)
...
Body DTO: [SAMLConfigurationApiDTO](#samlconfigurationapidto)

Return DTO: [SAMLConfigurationApiDTO](#samlconfigurationapidto)
...
JavaScript Usage ...
	...
	result = client.deleteFavoriteScopesByUser(user_Uuid, scope_Uuid)

REST method and path ...

	DELETE /users/{user_Uuid}/favoritescopes/{scope_Uuid}
...
| **environment_type** | No | string | Environment type | CLOUD, HYBRID, ONPREM |
...
| **ascending** | No | boolean | Whether to arrange in ascending order. | - |
| **cursor** | No | string | When making subsequent calls to retrieve additional results,this is the cursor returned by the last call to this method. | - |
| **environment_type** | No | string | filter by Environment type | CLOUD, HYBRID, ONPREM |
| **limit** | No | integer | The maximum number of items to return. Must be a positive integer. If not set, the server will provide a default. | - |
| **order_by** | No | string | The field to order the results by. | creation_date, name, risk_category, savings, severity |
...
JavaScript Usage ...
	...
	result = client.getFavoriteScopesByUser(user_Uuid)

REST method and path ...

	GET /users/{user_Uuid}/favoritescopes
...
## GetHistoricalContextByUuid

Get historical context for a given action_uuid

JavaScript Usage ...

	result = client.getHistoricalContextByUuid(action_Uuid)

REST method and path ...

	GET /actions/{action_Uuid}/context
...
| **environment_type** | No | string | Limit query to tags in the following environment | CLOUD, HYBRID, ONPREM |
...
Body DTO: [SettingApiInputDTO](#settingapiinputdto)
...
| `nextOccurence` | string | No | The date of the next occurrence. | - |
...
| `automationSettingList` | [SettingApiDTO](#settingapidto)[] | No | List of automation settings | - |
| `osMigrationSettingList` | [SettingApiDTO](#settingapidto)[] | No | List of OS migration settings | - |
...
Model to describe http proxy settings, like username, password, port numberand host
...
| `portNumber` | string | No | proxy host port number | - |
...
| `valueType` | string | No | Type of the value this field holds | BOOLEAN, GROUP_SCOPE, LIST, NUMERIC, STRING |
...
| Property | Type | Required? | Description | Enum Values |
| -------- | ---- | --------- | ----------- | ----------- |
| `destination` | [BaseApiDTO](#baseapidto) | No | Destination of the migration | - |
...
| `projectionDay` | integer | No | Projection day, indicate when to apply the change | - |
| `source` | [BaseApiDTO](#baseapidto) | No | Source of the migration, entity or group | - |
...
| Property | Type | Required? | Description | Enum Values |
| -------- | ---- | --------- | ----------- | ----------- |
| `businessUnit` | [BaseApiDTO](#baseapidto) | No | Business account to plan destination | - |
| `className` | string | No | - | - |
| `displayName` | string | No | - | - |
| `links` | [Link](#link)[] | No | - | - |
| `market` | [BaseApiDTO](#baseapidto) | No | Plan market currently uploaded to plan destination | - |
| `numErrors` | integer | No | Number of errors on the plan repository | - |
| `uuid` | string | No | - | - |
...
## SAMLConfigurationApiDTO

Model to describe a SAML configuration.

| Property | Type | Required? | Description | Enum Values |
| -------- | ---- | --------- | ----------- | ----------- |
| `alias` | string | No | The SAML private key alias | - |
| `className` | string | No | - | - |
| `displayName` | string | No | - | - |
| `enabled` | boolean | Yes | is SAML enabled | - |
| `entityId` | string | Yes | The SAML entity ID | - |
| `externalIP` | string | Yes | The SAML external accessible IP | - |
| `links` | [Link](#link)[] | No | - | - |
| `password` | string | No | The SAML key store password | - |
| `uuid` | string | No | - | - |
...
| `links` | [Link](#link)[] | No | - | - |
| `nextOccurence` | string | No | The date and time of the next occurence of this schedule represented in ISO8601 (YYYY-MM-DDTHH:MM:SSZ). For example, 2019-04-04T19:15Z | - |
...
| `className` | string | No | - | - |
| `defaultValue` | string | No | Default value before any change | - |
| `displayName` | string | No | - | - |
...
| `uuid` | string | No | - | - |
| `value` | string | Yes | Value of the setting | - |
...
| `displayName` | string | No | - | - |
| `links` | [Link](#link)[] | No | - | - |
| `settings` | [SettingApiDTO](#settingapidto)[] | Yes | List of Settings contained in the Manager | - |
| `uuid` | string | No | - | - |
